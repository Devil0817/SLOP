...
def pretrain(neox_args):
    ...
    # summary(model, input_size=[(1, 2048, 64768)], batch_size=1, device="cpu")
...
def get_batch_pipe(data, neox_args, curr_scheduler=None):
    ...
    #print('================= tokens in machine 0=================',tokens.tolist()[0][:4])
    #print('================= tokens type =================')
    #print(type(tokens))
...
def forward_step(
    ...
    print("=========tokens in forward_step 0============",tokens)
    if timers is not None:
...
def get_model(neox_args, use_cache=False, hidden_size=768):
    """Build the model."""
    ...
    neox_args.use_mup = False
    neox_args.hidden_size = hidden_size
    # neox_args.intermediate_size = 2048
    print_rank_0("get GPT2 model ... ", neox_args.hidden_size, neox_args.intermediate_size)
    model = GPT2ModelPipe(
        neox_args=neox_args,
        num_tokentypes=0,
        parallel_output=True,
...
def setup_model_and_optimizer(neox_args, use_cache=False, iteration=None):
    ...
    print_rank_0("neox_args.hidden_dim = ", hidden_size)
    model = get_model(neox_args=neox_args, use_cache=use_cache, hidden_size=hidden_size)
    ...
    if neox_args.load is not None:
        import copy
        old_args = copy.deepcopy(neox_args)
        old_args.hidden_size = 768
        # neox_args.hidden_size = 768
        old_args.num_attention_heads = 6
        old_args.no_load_optim = True
        old_args.finetune = True
        print_rank_0("&&& new config args: ", old_args.hidden_size)
        pre_model = get_model(neox_args=old_args, use_cache=use_cache, hidden_size=768)
        pre_optimizer, pre_param_groups = get_optimizer(model=pre_model, neox_args=old_args)
      
        pre_model, _, _, _ = deepspeed.initialize(
            model=pre_model,
        ...
        pre_model.total_params = get_total_params(pre_model.module)
        print_rank_0(f' >pre model total params: {"{:,}".format(pre_model.total_params)}')
        for i in pre_model.state_dict():
            print_rank_0(i)
        
        # print_rank_0(pre_model)
        iters = load_checkpoint(neox_args=old_args,
            model=pre_model,
            optimizer=None,
            lr_scheduler=None,
            iteration=None,
        )
        print_rank_0("#################### pre model ############")
        print_rank_0(pre_model)
    ...
    if neox_args.load is not None and expand_flag is False:
        ...
        for name, param in pre_model.named_parameters():
            para_weight = pre_model.state_dict()[name]
            if "attention.query_key_value" in name:  #[3*hidden_dim, hidden_dim]
                step = int(neox_args.hidden_size)
                model.state_dict()[name][0:768, 0:768].copy_(para_weight[0:768,:])
                model.state_dict()[name][step:step+768, 0:768].copy_(para_weight[768:768*2,:])
                model.state_dict()[name][2*step:2*step+768, 0:768].copy_(para_weight[2*768:,:])
            elif "attention.dense" in name: #[hidden_dim, hidden_dim]
                model.state_dict()[name][0:768, 0:768].copy_(para_weight)
            elif "norm.scale" in name:#[hidden_dim]
                model.state_dict()[name][0:768].copy_(para_weight)
            elif "mlp" in name:
                if "w2" in name: #[hidden_dim, ffn_dim]
                    model.state_dict()[name][0:768, :].copy_(para_weight)
                else: #[ffn_dim, hidden_dim]
                    model.state_dict()[name][:, 0:768].copy_(para_weight)
            elif "rotary_emb" in name: #[64]
                dim = para_weight.shape[0]
                model.state_dict()[name][0:dim].copy_(para_weight)
            else:#[69120, hidden_dim]
                model.state_dict()[name][:, 0:768].copy_(para_weight)

            print_rank_0("ooo  ", name,"———— ", para_weight.shape)#, para_weight)
            print_rank_0("fff  ", model.state_dict()[name].shape)#, model.state_dict()[name])
            print_rank_0("\n")
                
        '''
        if int(str(model.device)[-1]) % 2 == 0:
            dir = "/zhangpai31a/CM-TEST/yanxr/test_eval/cm_out0/"
        else:
            dir = "/zhangpai31a/CM-TEST/yanxr/test_eval/cm_out1/"
        
        for name, param in model.named_parameters():
            if param.requires_grad:
                temp_pt = torch.load(dir + name + ".pt")
                model.state_dict()[name].copy_(temp_pt)
                print(name, param.data.shape, str(model.device))

        print("load CM_PT 999999999999999999999999999999999999", str(model.device))
        
        #load_llama
        if int(str(model.device)[-1]) % 2 == 0:
            bf_pt_dir = "/zhangpai31a/CM-TEST/yanxr/test_eval/bfout0/"
        else:
            bf_pt_dir = "/zhangpai31a/CM-TEST/yanxr/test_eval/bfout1/"

        for name, param in model.named_parameters():
            if param.requires_grad:
                if name == "module.0.word_embeddings.weight":
                    embedding_pt = torch.load(bf_pt_dir + "tok_embeddings.weight.pt")
                    model.state_dict()[name].copy_(embedding_pt)
                if name == "module.43.norm.scale":
                    norm_pt = torch.load(bf_pt_dir + "norm.weight.pt")
                    model.state_dict()[name].copy_(norm_pt)
                if name == "module.44.final_linear.weight":
                    final_linear_pt = torch.load(bf_pt_dir + "output.weight.pt")
                    model.state_dict()[name].copy_(final_linear_pt)

                if "input_layernorm" in name:
                    if name[8] == ".":
                        lay = int(name[7])
                    else:
                        lay = int(name[7]) * 10 + int(name[8])
                    model_layernorm_layer = "module." + str(lay-2) + ".attention_norm.weight.pt"
                    layernorm_layer_pt = torch.load(bf_pt_dir + model_layernorm_layer)
                    model.state_dict()[name].copy_(layernorm_layer_pt)
                ...
                if "query_key_value" in name:
                    if name[8] == ".":
                        ...
                        lay = int(name[7]) * 10 + int(name[8])
                    load_qkv_name = "layers." + str(lay-2) + ".attention.qkv.weight.pt"
                    qkv_layer = torch.load(bf_pt_dir + load_qkv_name)
                    model.state_dict()[name].copy_(qkv_layer)
                ...
                if "attention.dense" in name:
                    ...
                    attention_dense_name = "layers." + str(lay-2) + ".attention.wo.weight.pt"
                    attention_dense = torch.load(bf_pt_dir + attention_dense_name)
                    model.state_dict()[name].copy_(attention_dense)
                ...
                if "post_attention_layernorm" in name:
                    if name[8] == ".":
                        ...
                        lay = int(name[7]) * 10 + int(name[8])
                    attention_norm_name = "layers." + str(lay-2) + ".attention_norm.weight.pt"
                    attention_norm = torch.load(bf_pt_dir + attention_norm_name)
                    model.state_dict()[name].copy_(attention_norm)
                ...
                if "mlp.w1" in name:
                    if name[8] == ".":
                        ...
                        lay = int(name[7]) * 10 + int(name[8])
                    w1_name = "layers." + str(lay-2) + ".feed_forward.w1.weight.pt"
                    w1 = torch.load(bf_pt_dir + w1_name)
                    model.state_dict()[name].copy_(w1)
                ...
                if "mlp.w2" in name:
                    if name[8] == ".":
                        ...
                        lay = int(name[7]) * 10 + int(name[8])
                    w2_name = "layers." + str(lay-2) + ".feed_forward.w2.weight.pt"
                    w2 = torch.load(bf_pt_dir + w2_name)
                    model.state_dict()[name].copy_(w2)
                ...
                if "mlp.w3" in name:
                    if name[8] == ".":
                        ...
                        lay = int(name[7]) * 10 + int(name[8])
                    w3_name = "layers." + str(lay-2) + ".feed_forward.w3.weight.pt"
                    w3 = torch.load(bf_pt_dir + w3_name)
                    model.state_dict()[name].copy_(w3)
        '''
...
def train(
    ...
    # skip_steps = [[3, 6], [10, 13], [20, 27]] 默认[]
    skip_steps = neox_args.skip_steps
    steps_tile = []
    if skip_steps:
        for steps in skip_steps:
            start = steps[0]
            end = steps[1]
            for i in range(start, end + 1):
                steps_tile.append(i)
    ####test      steps       should   delete
    while iteration < neox_args.train_iters:
       # if iteration <= 36174 or iteration>= 36186: #no skip
        #if iteration not in steps_tile and iteration != 0: # no skip
        if iteration not in steps_tile: # no skip
            ...
            # print(model.get_global_grad_norm())
        else:
