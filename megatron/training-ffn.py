...
def get_model(neox_args, use_cache=False, intermediate_size=2048):
    """Build the model."""
    ...
    neox_args.use_mup = False
    neox_args.intermediate_size = intermediate_size
    print_rank_0("get GPT2 model ... ", neox_args.intermediate_size)
    model = GPT2ModelPipe(
        neox_args=neox_args,
        num_tokentypes=0,
        parallel_output=True,
...
def setup_model_and_optimizer(neox_args, use_cache=False, iteration=None):
    ...
    model = get_model(neox_args=neox_args, use_cache=use_cache, intermediate_size=neox_args.itermediate_size)
    ...
    if neox_args.load is not None:
        ...
        old_args.itermediate_size = 2048
        neox_args.itermediate_size = 2048
        old_args.no_load_optim = True
        old_args.finetune = True
        print_rank_0("&&& new config args: ", old_args.itermediate_size, old_args.hidden_size, neox_args.itermediate_size)
        pre_model = get_model(neox_args=old_args, use_cache=use_cache, intermediate_size=2048)
    ...
    if neox_args.load is not None and expand_flag is False:
        ...
        for name, param in pre_model.named_parameters():
            ...
            if "mlp.w" not in name:
                model.state_dict()[name].copy_(para_weight)
            elif "w2" in name:  #[hidden_dim, ffn_dim]
                model.state_dict()[name][:,0:old_args.itermediate_size].copy_(para_weight)
            else:
                #[ffn_dim, hidden_dim]
                model.state_dict()[name][0:old_args.itermediate_size,:].copy_(para_weight)
