...
# ENDOFTEXT = "<|endoftext|>"
SPECIAL_TOKENS = ["<|endoftext|>", "<|padding|>", "<unk>", "<s>", "</s>", "[cls]", "[seq]", "[mask]"]
PAT_STR = r"""(?i:'s|'t|'re|'ve|'m|'ll|'d)|[^\r\n\p{L}\p{N}]?\p{L}+|\p{N}{1,3}| ?[^\s\p{L}\p{N}]+[\r\n]*|\s*[\r\n]+|\s+(?!\S)|\s+"""
# PAT_STR = r"""(?i:'s|'t|'re|'ve|'m|'ll|'d)|[^\\r\\n\\p{L}\\p{N}]?\\p{L}+|\\p{N}{1,3}| ?[^\\s\\p{L}\\p{N}]+[\\r\\n]*|\\s*[\\r\\n]+|\\s+(?!\\S)|\\s+"""
SPECIAL_START_ID = 159251
special_token_ranks = {}
for token in SPECIAL_TOKENS:
    special_token_ranks[token] = SPECIAL_START_ID
    SPECIAL_START_ID += 1
...
def _load_tiktoken_bpe(tiktoken_bpe_file):
...
class TiktokenTokenizer(AbstractTokenizer):
    ...
    def __init__(self, vocab_file, **kwargs,):
        ...
        # self.special_tokens = special_token_ranks
        self.tokenizer = tiktoken.Encoding("Jiutian",
            pat_str=PAT_STR,
        ...
        self.pad_id = special_token_ranks["<|padding|>"]
        self.unk_id = special_token_ranks["<unk>"]
        self.bos_id = special_token_ranks["<s>"]
        self.eos_id = special_token_ranks["</s>"]
        self.cls_id = special_token_ranks["[cls]"]
        self.seq_id = special_token_ranks["[seq]"]
        self.mask_id = special_token_ranks["[mask]"]
    ...
    def tokenize(self, text: str, allowed_special:str = "all"):
        return self.tokenizer.encode(text,  allowed_special=allowed_special)

    def tokenize_batch(self, text_batch: List[str], allowed_special:str = "all"):
        return self.tokenizer.encode_batch(text_batch, allowed_special=allowed_special)
